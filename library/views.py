from .utils import BookSorterService
...
class HomeView(ListView):
    ...
    def get_queryset(self):
        ...
        search_query = self.request.GET.get('searchBook')
        sort_query = self.request.GET.get('sort')
        if search_query:
            queryset = queryset.filter(title__icontains=search_query)
        strategy_helper = BookSorterService()
        queryset = strategy_helper.sort_books(queryset, sort_query)
        return queryset

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context.update({
            'searchTerm': self.request.GET.get('searchBook', ''),
...
def change_availability(request, book_id):
    ...
    if book.available:
        ...
        # Establecer la disponibilidad en 31 días desde hoy
        book.availability = datetime.date.today() + datetime.timedelta(days=31)
    else:
        book.available = True
...
def reserve_book(request, book_id):
    ...
    if book.reserved:
        ...
        book.reserved = True
        # Establecer la fecha de reserva en 7 días desde hoy
        book.reserved_date = datetime.date.today() + datetime.timedelta(days=7)
...
def register_user(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Redirigir a la página de edición de usuarios
...
def editar_usuario(request, user_id):
    ...
    if request.method == 'POST':
        ...
        if formulario.is_valid():
            ...
            # Obtener el usuario pero no guardar en la base de datos todavía
